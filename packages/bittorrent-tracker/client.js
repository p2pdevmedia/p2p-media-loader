 ...
 * @param {object} opts.stats                    stats of the downloading prosses (uploaded, downloaded, left)
...
class Client extends EventEmitter {
   ...
   */
  update (opts) {
    console.log('update at client.js',opts)
    opts = this._defaultAnnounceOpts(opts)
    if (opts.event) delete opts.event
    debug('send `update` %o', opts)
    opts.stats = {'data':'update'}
    console.log('update with stats',opts)
    this._announce(opts)
  }

    /**
   * Send a `stats` to the trackers with downloaded data.
   * @param {Object} opts
   * @param {number=} opts.p2p
   * @param {number=} opts.http
   * @param {number=} opts.total 
   * @param {number=} opts.percentaje
   * */
  stats (opts) {
    if (!opts) opts = {}
    opts = this._defaultAnnounceOpts(opts)
    opts.event = 'stats'
    debug('send `stats` %o', opts)
    this._trackers.forEach(tracker => {
